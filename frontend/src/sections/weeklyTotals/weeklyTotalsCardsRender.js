...
function WeeklyFoodSalesCardRender({ teamMembers, difference, positive = false, selectedDate, sx }) {
	if (!teamMembers) {
		return <CircularProgress />;
	}

	const totalWeeklyFoodSales = teamMembers.reduce((total, member) => {
		return total + member.weeklyTotals.reduce((sum, dayTotal) => sum + dayTotal.foodSales, 0);
	}, 0);
		...
		<Card sx={sx}>
			<CardContent>
				<Stack alignItems="flex-start" direction="row" justifyContent="space-between" spacing={3}>
					<Stack spacing={1}>
						<Typography color="text.secondary" variant="overline">
							Total Weekly Food Sales ({startDate} - {endDate})
						</Typography>
